...
namespace graphsearch_tests
{
    ...
    public class Tests
    {
        ...
        /// <returns></returns>
        private static string DoRun(string[] args)
        {
            //all code is from program.cs however it has been modified slightly to ensure it can be tested fully
            string result = "";
            StringWriter failure = new();//ensures any console outputs are caught and can be checked by the assertion
            ...
            {
                GetInformation.GrabData(args, out string startNode, out string endNode, out string fileToRead, out string fileToWrite, out bool outputToConsole, out bool debugInfo, out GetInformation.SortingAlgorithm chosenAlgorithm, out bool failedBuild);
                if (GetInformation.CanRun(failedBuild, startNode, endNode, fileToRead, chosenAlgorithm))
                {
                    if (GetInformation.ParseFile(fileToRead, out List<Node> nodes, out int[,] adjacencyMatrix))
                    {
                        if (GetInformation.SetStartNodeAndEndNode(startNode, endNode, nodes, out Node trueStartNode, out Node trueEndNode))
                        {
